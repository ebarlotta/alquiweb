<div>
    ...
    <div class="d-flex">
        <h2 class="col-2"> Garantes </h2>
        <input class="ml-3 btn btn-info col-1" type="button" value="Nuevo" wire:click="Nuevo()" data-toggle="modal" data-target="#ModalNuevaGarante">
    ...
    @if($garantes_fisica)
        <table class="table table-stripped">
            <tr>
                ...
                {{-- <{{ $garantes_fisica->links() }}> --}}
            </tr>    
            @foreach($garantes_fisica as $garante)
                <tr>
                    <td>{{ $garante->apellidos }}, {{ $garante->nombres }}</td>
                    <td>{{ $garante->telefono }}</td>
                    <td>{{ $garante->email_id }}</td>
                    <td>{{ $garante->calle }} - {{ $garante->provinciadescripcion }} - {{ $garante->localidaddescripcion }}</td>
                    <td>
                        <input class="btn btn-warning" value="Modificar" wire:click="edit({{ $garante->persona_id }},'fisica')"  data-toggle="modal"  data-target="#ModalNuevaGarante">
                        <input class="btn btn-danger" value="Eliminar" wire:click="CargarDatosParaBorrar({{ $garante->persona_id }},'{{ $garante->persona_type }}')" data-toggle="modal" data-target="#ModalEliminarGarante">
                    </td>
                </tr>
            @endforeach
        </table>
    ...
    @if($garantes_juridica)
        <table class="table table-stripped">
            <tr>
                ...
                <td class="enc">Domicilio</td>
                <td class="enc">Opciones</td>
                {{-- <{{ $garantes_juridica->links() }} --}}
            </tr>  
            @foreach($garantes_juridica as $garante)
                <tr>
                    <td>{{ $garante->razonsocial }}</td>
                    <td>{{ $garante->telefono }}</td>
                    <td>{{ $garante->email_id }}</td>
                    <td>{{ $garante->calle }} - {{ $garante->provinciadescripcion }} - {{ $garante->localidaddescripcion }}</td>
                    <td>
                        <input class="btn btn-warning" value="Modificar" wire:click="edit({{ $garante->persona_id }},'juridica')" data-toggle="modal" data-target="#ModalNuevaGarante">
                        <input class="btn btn-danger" value="Eliminar" wire:click="CargarDatosParaBorrar({{ $garante->persona_id }},'{{ $garante->persona_type }}')" data-toggle="modal" data-target="#ModalEliminarGarante">
                    </td>
                </tr>
            @endforeach
    ...
    <div wire:ignore.self class="modal fade" id="ModalNuevaGarante" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document" style="max-width: 70%; ">
            <div class="modal-content" style="width: inherit">
                <div class="modal-header">
                    <h5 class="modal-title">Datos del Garante</h5>
    ...
    <div wire:ignore.self class="modal fade" id="ModalEliminarGarante" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document" style="max-width:20%">
            <div class="modal-content" style="width: inherit">
                <div class="modal-header">
                    <h5 class="modal-title">Eliminar Garante</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="px-3 py-3">
                    <div  wire:ignore.self class="col-12 d-flex">
                        <div class="text-center m-2 p-2 rounded-md"style="background-color: lightgray;border-radius: 5px;">
                            <div class="input-group mx-auto">
                                <label for="">Seguro que quiere eliminar al Garante?</label>
                            </div>
